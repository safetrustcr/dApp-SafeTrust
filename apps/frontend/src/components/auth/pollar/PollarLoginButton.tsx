...
export function PollarLoginButton({ onWalletReady }: PollarLoginButtonProps) {
  ...
  if (isAuthenticated && wallet?.address) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-green-300 bg-green-50 px-4 py-3 text-sm font-medium text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300">
        <span>✓</span>
        <span>
          Wallet ready — {wallet.address.slice(0, 6)}…{wallet.address.slice(-4)}
        </span>
      </div>
    );
  }
